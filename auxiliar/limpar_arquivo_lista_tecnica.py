...
def resolver_caminho_existente(caminho: Path) -> Path:
    if caminho.exists():
        return caminho

    texto = str(caminho)
    if len(texto) >= 3 and texto[1:3] in {":\\", ":/"}:
        restante = texto[1:]
        for unidade in ("Z", "Y"):
            candidato = Path(f"{unidade}{restante}")
            if candidato.exists():
                return candidato
...
def preparar_lista_tecnica(paths_csv: Path, output_csv: Path) -> tuple[pd.DataFrame, list[str]]:
    ...
    for indice, caminho in enumerate(caminhos, start=1):
        ...
        origem = resolver_caminho_existente(Path(caminho_str))
        if not origem.is_absolute() and not origem.exists():
            origem = resolver_caminho_existente((PROJECT_DIR / origem).resolve())
