(() => {
  ...
  function renderSecao(secao) {
    ...
    const linhas = secao.linhas.map((item) => { const desempenho = Number(item.quantidade_deveria_produzida || 0) > 0 ? Number(item.quantidade_apontada_dia || 0) / Number(item.quantidade_deveria_produzida) * 100 : 0; return `<article class="painel-exibicao-line"><header class="painel-exibicao-line__header"><div><h3>${esc(item.linha)}</h3><p class="painel-exibicao-line__op">OP ${esc(item.op)} · ${esc(item.codigo)} · ${esc(item.descricao)}</p></div><span class="painel-exibicao-line__status">${esc(item.status || "Em fila")}</span></header><div class="painel-exibicao-line__metrics"><div><span>Programada</span><strong>${fmt(item.quantidade_programada)}</strong></div><div><span>Apontado</span><strong>${fmt(item.quantidade_apontada)}</strong></div><div><span>Apontado dia</span><strong>${fmt(item.quantidade_apontada_dia)}</strong></div><div><span>Deveria produzir</span><strong>${item.metricas_disponiveis ? fmt(item.quantidade_deveria_produzida) : "-"}</strong></div><div><span>Desempenho</span><strong>${item.metricas_disponiveis ? `${fmt(desempenho)}%` : "-"}</strong></div></div><div class="painel-exibicao-line__timeline"><div><span>Previsão de saída</span><strong>${esc(item.previsao_saida)}</strong></div><div><span>Início da fila</span><strong>${esc(item.hora_inicio_fila)}</strong></div><div><span>Último apontamento</span><strong>${esc(item.ultimo_apontamento)}</strong></div></div></article>`; }).join("");
    palco.innerHTML = `<header class="painel-exibicao-section__header"><div><p class="eyebrow">SEÇÃO EM PRODUÇÃO</p><h2>${esc(secao.titulo)}</h2><p>${secao.linhas.length} linha(s) ativa(s)</p></div><span class="painel-exibicao-count">${secao.linhas.length} linhas</span></header><div class="${classeGrade}">${linhas}</div><div class="painel-exibicao-line__observation"><span>Última observação da seção</span><p>${esc(observacao)}</p></div>`;
  }
