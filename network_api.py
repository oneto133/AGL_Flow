...
def get_local_access_url() -> str:
    """Retorna uma URL vÃ¡lida para abrir no navegador localmente."""
...
def root(request: Request):
    """Carrega a tela de acesso inicial da estaÃ§Ã£o secundÃ¡ria."""
    return templates.TemplateResponse(request, "login.html")


@app.get("/reposicao", response_class=HTMLResponse)
def reposicao_page(request: Request):
    """Carrega a interface principal da aplicaÃ§Ã£o na estaÃ§Ã£o secundÃ¡ria."""
    return templates.TemplateResponse(request, "index.html")


@app.get("/config-etiquetas", response_class=HTMLResponse)
def label_config_page(request: Request):
    """Carrega a tela de configuraÃ§Ã£o de etiquetas da interface principal."""
    return templates.TemplateResponse(request, "label_config.html")


@app.get("/health")
def health():
    """Verifica se a API local estÃ¡ ativa e qual servidor remoto estÃ¡ sendo usado."""
    return {
        "status": "ok",
...
def set_remote_target(payload: RemoteTargetRequest):
    """Permite ajustar a URL do servidor remoto em tempo de execuÃ§Ã£o."""
...
def proxy_items(target_url: str | None = None):
    """Encaminha a listagem de itens do servidor para a estaÃ§Ã£o secundÃ¡ria."""
...
def proxy_base_item(codigo: str, target_url: str | None = None):
    """Encaminha a busca de um cÃ³digo na base do servidor remoto."""
...
def proxy_config(target_url: str | None = None):
    """Encaminha a configuraÃ§Ã£o do servidor para a mÃ¡quina secundÃ¡ria."""
...
def proxy_label_config(target_url: str | None = None):
    """Encaminha a configuraÃ§Ã£o de etiquetas do servidor para a estaÃ§Ã£o secundÃ¡ria."""
...
def proxy_set_label_config(payload: dict[str, Any], target_url: str | None = None):
    """Envia alteraÃ§Ãµes de etiquetas para o servidor remoto."""
...
def proxy_print(payload: PrintRequest, target_url: str | None = None):
    """Envia uma impressÃ£o para o servidor remoto."""
...
def proxy_print_all(target_url: str | None = None):
    """Envia a impressÃ£o em lote para o servidor remoto."""
    _, body = _proxy_request("POST", "/api/imprimir-todos", target_url=target_url)
    return body


def main():
    """Entrada da API secundÃ¡ria para uso em rede."""
    ...
    try:
        ...
        print("NÃ£o foi possÃ­vel abrir o navegador automaticamente. Use esta URL manualmente:")
        print(browser_url)
